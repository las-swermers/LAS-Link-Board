    ...
    if (lowerText === 'help' || lowerText === 'commands') {
      return res.json({
        text: '*LinkBoard Commands:*\n' +
          ...
          '• `/general` `/followup` `/transport` — any category name works\n' +
          '• `note [text]` — add a to-do note (uncategorized)\n' +
          '• Use `@student name` to tag a student'
      });
    }

    // ── Slash category commands: /medical, /admin, /general, /followup, /transport, etc. ──
    const slashMatch = cleanText.match(/^\/(\w[\w-]*)\s+([\s\S]+)/);
    if (slashMatch) {
      const slashCat = slashMatch[1].toLowerCase().replace(/-/g, ' ');
      const slashNote = slashMatch[2].trim();
