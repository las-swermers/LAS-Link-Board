...
module.exports = async (req, res) => {
  ...
  // ── Normalize Google Workspace Add-on event format ──
  // When the Chat App is registered as a Workspace Add-on (service account:
  // gcp-sa-gsuiteaddons.iam.gserviceaccount.com), Google sends events with
  // commonEventObject at the top level instead of the standalone Chat App format.
  if (body && body.commonEventObject && !body.type) {
    // Authorization event — sent when the user hasn't yet authorized the Add-on.
    // Respond with a renderActions card so Google Chat accepts the response.
    if (body.authorizationEventObject) {
      console.log('Authorization event received — user needs to authorize the Add-on');
      return res.status(200).json({
        renderActions: {
          action: {
            navigations: [{
              pushCard: {
                header: { title: 'LinkBoard', subtitle: 'Authorization required' },
                sections: [{
                  widgets: [
                    {
                      textParagraph: {
                        text: 'LinkBoard needs to verify your account before it can respond to commands.'
                      }
                    },
                    {
                      buttonList: {
                        buttons: [{
                          text: 'Open LinkBoard',
                          onClick: { openLink: { url: 'https://las-link-board.vercel.app' } }
                        }]
                      }
                    }
                  ]
    ...
    // Regular message event in Workspace Add-on format — normalize to standalone format.
    // Add-on format nests the data under body.chat instead of the top level.
    if (body.chat) {
      const chatEvent = body.chat;
      const messagePayload = chatEvent.messagePayload || {};
      const msg = messagePayload.message || {};

      // Carry over slash command metadata if the message object doesn't already have it
      if (chatEvent.slashCommandMetadata && !msg.slashCommand) {
        msg.slashCommand = { commandId: String(chatEvent.slashCommandMetadata.commandId || '') };
        if (!msg.argumentText) msg.argumentText = '';
      }

      body = {
        type: chatEvent.type || (messagePayload.message ? 'MESSAGE' : 'ADDED_TO_SPACE'),
        message: msg,
        space: chatEvent.space || messagePayload.space || {},
        user: chatEvent.user || messagePayload.user || {}
      };
      console.log('Normalized Add-on event → type:', body.type, '| user:', (body.user || {}).email || 'unknown');
    }
  }

  const event = body || {};
  const eventType = event.type;
  const spaceType = (event.space && event.space.type) || ''; // DM or ROOM/SPACE

  console.log('Chat bot event:', eventType, '| space:', spaceType, '| user:', (event.user || {}).email || 'unknown',
    '| body type:', typeof body, '| body keys:', Object.keys(event).join(',') || 'none',
    '| raw body type:', typeof req.body);
